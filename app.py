...
class PDFAccessibility(Stack):
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        pdf_processing_vpc.add_interface_endpoint(
        ...
        pdf_processing_vpc.add_gateway_endpoint(
            "S3Endpoint", service=ec2.GatewayVpcEndpointAwsService.S3
        )

        # ECS Cluster
        pdf_remediation_cluster = ecs.Cluster(
            self, "PdfRemediationCluster", vpc=pdf_processing_vpc
        )
        ...
        ecs_task_role.add_to_policy(
            iam.PolicyStatement(
                ...
                resources=[
                    f"arn:aws:secretsmanager:{region}:{account_id}:secret:/myapp/*"
                ],
            )
        )
        ...
        adobe_autotag_container_def = adobe_autotag_task_def.add_container(
        ...
        # alt_text_container_def = alt_text_task_def.add_container(
        #     "alt-text-llm-container",
        #     image=ecs.ContainerImage.from_registry(
        #         alt_text_generator_image_asset.image_uri
        #     ),
        #     memory_limit_mib=1024,
        #     logging=ecs.LogDrivers.aws_logs(
            ...
            container_overrides=[
                tasks.ContainerOverride(
                    ...
                    environment=[
                        ...
                        tasks.TaskEnvironmentVariable(
                            name="S3_FILE_KEY", value=sfn.JsonPath.string_at("$.s3_key")
                        ),
                        tasks.TaskEnvironmentVariable(
                            name="S3_CHUNK_KEY",
                            value=sfn.JsonPath.string_at("$.chunk_key"),
                        ),
                        tasks.TaskEnvironmentVariable(name="AWS_REGION", value=region),
                    ],
                )
            ],
        ...
        #                     name="S3_FILE_KEY",
        #                     value=sfn.JsonPath.string_at(
        #                         "$.Overrides.ContainerOverrides[0].Environment[1].Value"
        #                     ),
        #                 ),
        #                 tasks.TaskEnvironmentVariable(name="AWS_REGION", value=region),
        #             ],
        ...
        pdf_chunks_map_state.iterator(adobe_autotag_task)
        # next(alt_text_generation_task)
        ...
        pdf_merger_lambda = lambda_.Function(
            ...
            code=lambda_.Code.from_asset(
                "lambda/pdf-merger-lambda/PDFMergerLambda/target/PDFMergerLambda-1.0-SNAPSHOT.jar"
            ),
            environment={
        ...
        pdf_merger_lambda_task = tasks.LambdaInvoke(
            ...
            payload=sfn.TaskInput.from_object({"fileNames.$": "$.chunks[*].s3_key"}),
        ...
        parallel_accessibility_workflow = sfn.Parallel(
            self, "ParallelAccessibilityWorkflow", result_path="$.ParallelResults"
        )
        parallel_accessibility_workflow.branch(remediation_chain)
        parallel_accessibility_workflow.branch(
            pre_remediation_accessibility_checker_task
        )
        ...
        pdf_remediation_state_machine = sfn.StateMachine(
            ...
            logs=sfn.LogOptions(
                destination=pdf_remediation_workflow_log_group, level=sfn.LogLevel.ALL
            ),
        )
        ...
        pdf_splitter_lambda.add_environment(
            "STATE_MACHINE_ARN", pdf_remediation_state_machine.state_machine_arn
        )
        # Store log group names dynamically
        pdf_splitter_lambda_log_group_name = (
            f"/aws/lambda/{pdf_splitter_lambda.function_name}"
        )
        ...
        post_remediation_checker_log_group_name = (
            f"aws/lambda/{post_remediation_accessibility_checker.function_name}"
        )
        ...
        dashboard.add_widgets(
            cloudwatch.LogQueryWidget(
                ...
                log_group_names=[
                    pdf_splitter_lambda_log_group_name,
                    pdf_merger_lambda_log_group_name,
                    adobe_autotag_log_group.log_group_name,
                    alt_text_generator_log_group.log_group_name,
                ],
